import { buildin_struct as buildin, format, jsonOption, } from "./tars";

function structToTars(scope, typename, options) {
  // debugger;
  let typeMap = {};
  let typeDeclare = '';
  let parentKey = '';
  ...
  function jsToGo0(scope, typename, options) {
    // debugger;
    let ci = "type,void,struct,bool,byte,short,int,double,float,long,string,vector,map,key,routekey,module,interface,out,require,optional,false,true,enum,const";
    let ciar = ci.split(",");
    ...
    function parseStruct(object, depth = 0, omitempty = undefined, nullable = undefined) {
      ...
      for (let i in keys) {
        if (keys.hasOwnProperty(i)) {
          ...
          append(" " + (ciar.indexOf(keyName)<0 ? keyName : format(keyName)) + ';');
